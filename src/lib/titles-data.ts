...
/**
 * Aqui é onde você garante que o GitHub Pages sempre carrega as capas.
 * Adiciona aos poucos os posters que quiser.
 */
const POSTER_BY_SLUG: Record<string, string> = {
  // ✅ os 6 que aparecem na home (do teu print)
...
function posterFromSlug(slug: string): string | null {
  const path = POSTER_BY_SLUG[slug];
...
export const ORIGINAL_TITLES_DATA: Title[] = SEED_TITLES.map((t) => {
  ...
  return {
    ...
    posterUrl: posterFromSlug(slug) || "https://placehold.co/500x750?text=Sem+Capa",
  };
});
